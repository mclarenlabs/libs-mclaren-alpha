...
- (void) mouseDragged:(NSEvent*)event
{
  ...
  [self performSelectorOnMainThread: @selector(_asyncSendKeyPressure:)
			 withObject: @[@(_midiNote), @(udistance)]
		      waitUntilDone: NO
   ];
}

// Note: during drags, without asynchrony the Transcript blocks if
// logging events.  This asynchrony helps.

- (void) _asyncSendKeyPressure:(NSArray*)args {
  int midiNote = [args[0] integerValue];
  unsigned vel = [args[1] unsignedIntegerValue];
  [self sendKeyPressure:midiNote vel:vel];
...
{
  applyWithTwoInts(_noteOnTarget, _noteOnSelector, midiNote, vel);
}

- (void) sendNoteOff:(unsigned)midiNote vel:(unsigned)vel
{
  applyWithTwoInts(_noteOffTarget, _noteOffSelector, midiNote, vel);
}

- (void) sendKeyPressure:(unsigned)midiNote vel:(unsigned)vel
{
  applyWithTwoInts(_keyPressureTarget, _keyPressureSelector, midiNote, vel);
}
...
- (void) onNoteOn:(id)block {
  [self onNoteOn:@selector(value:value:) target:block];
}
...
- (void) onNoteOff:(id)block {
  [self onNoteOff:@selector(value:value:) target:block];
}
...
- (void) onKeyPressure:(id)block {
  [self onKeyPressure:@selector(value:value:) target:block];
}
