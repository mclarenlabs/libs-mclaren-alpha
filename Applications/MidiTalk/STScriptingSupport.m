...
{
  NSLog(@"_loadAppTalkAndRetryAction:with:");
    static BOOL isIn = NO;
...
{
  NSLog(@"_loadAppTalkAndRetryAction");
    static BOOL isIn = NO;
    id          retval = nil;
    ...
    // TOM:
    // if([self initializeApplicationScripting])
    if([NSApp initializeApplicationScripting])
    {
      // TOM:
      // retval = [self performSelector:sel];
      retval = 0;
      if ([self respondsToSelector:sel]) {
	NSLog(@"sel responds to selector:%d", sel);
	retval = [NSApp performSelector:sel];
      }
    }
