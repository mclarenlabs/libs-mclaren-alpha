...
- (void) sendNoteOn:(unsigned)midiNote vel:(unsigned)vel
{
  if (_target != nil) {
    if ([_target respondsToSelector:_noteOnSelector] == YES) {
      ...
      IMP imp = [_target methodForSelector:_noteOnSelector];
      imp(_target, _noteOnSelector, midiNote, vel);
    }
  }
}
...
- (void) sendNoteOff:(unsigned)midiNote vel:(unsigned)vel
{
  if (_target != nil) {
    if ([_target respondsToSelector:_noteOffSelector] == YES) {
      ...
      IMP imp = [_target methodForSelector:_noteOffSelector];
      imp(_target, _noteOffSelector, midiNote, vel);
    }
  }
}
...
- (void) sendKeyPressure:(unsigned)midiNote vel:(unsigned)vel
{
  ...
  if (_target != nil) {
    if ([_target respondsToSelector:_keyPressureSelector] == YES) {
      ...
      IMP imp = [_target methodForSelector:_keyPressureSelector];
      imp(_target, _keyPressureSelector, midiNote, vel);
    }
  }
  // NS_HANDLER
